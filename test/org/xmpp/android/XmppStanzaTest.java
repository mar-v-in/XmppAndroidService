...
public class XmppStanzaTest {
	...
	public void testEscapeAttributeValue() throws Exception {
		Assert.assertEquals(XmppStanza.escapeAttributeValue("<an:xml elem=\"can do\" much='here' />"),
	...
	public void testSingleAttributedBeginTag() throws Exception {
		...
		Assert.assertEquals(XmppStanza.buildBeginTag("each", "one", attributes), "<each:one blub=\"7\">");

		attributes = new HashMap<String, String>();
		attributes.put("good", "enc=&od>e\"d");
		DummyXmppStanza dummyXmppElement = new DummyXmppStanza("must", "work", attributes);
		Assert.assertEquals(dummyXmppElement.buildBeginTag(), "<must:work good=\"enc=&amp;od&gt;e&quot;d\">");
	}
	...
	public void testBuildBeginTagSimple() throws Exception {
		...
		Assert.assertEquals(dummyXmppElement.buildBeginTag(), "<one:more>");
	}
